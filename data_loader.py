...
# ========== МАКРОЭКОНОМИЧЕСКИЕ КОЭФФИЦИЕНТЫ РОССИИ (2015–2026) ==========
# Учитывают кризисы, санкции, падение/рост реальных зарплат, спрос на ИТ и т.д.
def get_russian_macro_factor(year, month=1):
    """
    Возвращает поправочный коэффициент для ЗАРПЛАТ и СПРОСА
    на основе реальных событий в России.
    Коэффициент 1.0 = базовый 2015 год.
    """
    # Базовые коэффициенты по годам (с учётом кризисов)
    macro_salary = {
        2015: 1.00,  # Кризис, падение нефти, санкции 2014
        2016: 0.97,  # Продолжение стагнации
        2017: 0.98,  # Восстановление слабое
        2018: 1.00,  # ЧМ-2018, небольшой рост
        2019: 1.02,  # Стагнация перед ковидом
        2020: 0.96,  # Пандемия COVID-19, падение доходов
        2021: 1.00,  # Восстановление
        2022: 0.92,  # Февраль 2022 — санкции, обвал рубля, инфляция
        2023: 0.94,  # Адаптация, но зарплаты реально снижены
        2024: 0.98,  # Постепенный рост, дефицит кадров
        2025: 1.02,  # Рост зп в IT и промышленности
        2026: 1.04  # Прогнозный рост (инфляция + дефицит)
    }

    # Корректировка по месяцам внутри года (плавная)
    month_factor = 1 + (month - 1) * 0.0015  # незначительный рост внутри года

    base = macro_salary.get(year, 1.00)
    return base * month_factor


def get_russian_demand_factor(year):
    """
    Коэффициент для СПРОСА на специалистов.
    В кризисы спрос падает, после — растёт.
    """
    demand_map = {
        2015: 0.85,  # Спад найма
        2016: 0.80,
        2017: 0.82,
        2018: 0.90,
        2019: 0.88,
        2020: 0.70,  # Ковидный провал
        2021: 0.85,
        2022: 0.65,  # Шок 2022 года
        2023: 0.75,
        2024: 0.85,
        2025: 0.95,
        2026: 1.00
    }
    return demand_map.get(year, 0.90)
...
# ========== ЭКСПЕРТНЫЕ ДАННЫЕ (без изменений) ==========
def get_expert_data():
    return pd.DataFrame([
...
# ========== РЕГИОНАЛЬНЫЕ ДАННЫЕ (без изменений) ==========
def get_regional_data():
    return {
        ...
        'Приморский край': {'salary': 101000, 'lat': 43.1, 'lon': 131.8, 'auto_factor': 0.85},
